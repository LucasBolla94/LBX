...
export default function SwapForm() {
  ...
  const fetchQuote = useCallback(async (amountToQuote: string) => {
    ...
    const outputMint = toToken === 'USDC' ? USDC_MINT : LBX_MINT;

    const rawAmount = Math.floor(Number(parseFormattedNumber(amountToQuote)) * 10 ** inputDecimals);
    ...
    try {
      ...
      const outAmount = Number(data.outAmount) / 10 ** outputDecimals;
      setQuoteAmount(outAmount.toLocaleString('en-US', {
  ...
  const executeSwap = async () => {
    if (!publicKey || !signTransaction) {
      return showMessage('⚠️ Connect a wallet compatible with signTransaction.');
    }

    if (!amount || Number(parseFormattedNumber(amount)) <= 0) {
      return showMessage('⚠️ Please enter a valid amount.');
    }
    ...
    try {
      ...
      if (err instanceof Error) {
        ...
        showMessage(err.message || 'Unexpected swap error occurred.');
      } else {
        console.error('Unknown Swap Error:', err);
  ...
  return (
    <>
      ...
      {/* Swap UI */}
      <div className="bg-[var(--background)] border border-[var(--border)] rounded-3xl shadow-lg p-6 sm:p-8 w-full max-w-xl mx-auto">
        ...
        {/* From Input */}
        <div className="mb-4">
          <label className="block text-sm sm:text-base font-medium text-[var(--foreground)] mb-2">You send</label>
          <div className="flex justify-between items-center border border-[var(--border)] rounded-xl px-4 py-3 bg-[var(--background)]/80">
            ...
            <input
              ...
              disabled={loading}
              className="bg-transparent text-right text-[var(--foreground)] text-2xl sm:text-3xl font-bold w-full ml-4 focus:outline-none tracking-wider placeholder:text-[var(--foreground)]/50"
            />
          </div>
        </div>
        ...
        <div className="flex justify-center my-4">
          <button
            onClick={handleSwap}
            disabled={loading}
            className="text-[var(--foreground)] hover:opacity-80 font-semibold text-base sm:text-lg px-6 py-2 rounded-full border border-[var(--border)] hover:bg-[var(--foreground)] hover:text-[var(--background)] transition"
          >
            🔄 Swap
          </button>
        </div>
        ...
        {/* To Output */}
        <div className="mb-6">
          <label className="block text-sm sm:text-base font-medium text-[var(--foreground)] mb-2">You receive</label>
          <div className="flex justify-between items-center border border-[var(--border)] rounded-xl px-4 py-3 bg-[var(--background)]/80">
        ...
        <button
          ...
          className="w-full bg-gradient-to-r from-green-500 to-emerald-600 text-white font-semibold text-base sm:text-lg py-3 rounded-full hover:brightness-110 transition disabled:opacity-50"
        >
          {loading
            ? toToken === 'LBXO'
              ? 'Buying LBXO...'
